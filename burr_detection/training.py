...
from burr_detection.utils import SmoothedValue, MetricLogger, set_seed


class YOLOTrainer:
    def __init__(self, model_size="yolo11n.pt", prints_per_epoch=5, ray_tune_callback=None):
        ...
        # Ultralytics is too noisy
        logging.getLogger("ultralytics").setLevel(logging.WARNING)
        
    def train(self, yolo_data_dir, output_dir, config=None):
        if config is None:
            config = {}
        ...
        base_path = Path(yolo_data_dir).absolute()
        dataset_config = {
            "path": str(base_path),
        ...
        training_steps = [
            {"batch": 8, "accumulate": 1, "min_epochs": 25, "patience": 15}, # patience is early stopping period
            {"batch": 8, "accumulate": 4, "min_epochs": 30, "patience": 15},
            {"batch": 8, "accumulate": 16, "min_epochs": 35, "patience": 15},
            {"batch": 8, "accumulate": 64, "min_epochs": 40, "patience": 15}
        ]
        ...
        train_txt_path = str(base_path / "train.txt")
        ...
        for step_idx in range(start_step, len(training_steps)):
            ...
            if step_idx == start_step and step_epochs_completed > 0:
                total_step_epochs = step["min_epochs"] + step["patience"] - step_epochs_completed
                if total_step_epochs <= 0:
                    continue
                print(f"Resuming step {step_idx+1} from epoch {step_epochs_completed+1}")
            else:
                total_step_epochs = step["min_epochs"] + step["patience"]
            
            self.epochs = total_step_epochs
            ...
            self.print_freq = max(1, batches_per_epoch // self.prints_per_epoch)
            ...
            self.model.add_callback("on_train_batch_end", self._on_batch_end)
            ...
            total_epochs += getattr(results, 'epoch', total_step_epochs)
                ...
                for _, row in df.iterrows():
                    ...
                    if precision + recall > 0:
                        ...
                        if f1 > best_f1:
                            best_f1 = f1
                            epoch = int(row["epoch"])
                            # Ultralytics saves last epoch as last.pt
                            if epoch == max_epoch:
                                candidate = weights_dir / "last.pt"
                            else:
                                candidate = weights_dir / f"epoch{epoch}.pt"
                            if candidate.exists():
                                final_weights = candidate

        if final_weights:
            best_model_path = str(final_weights)
            print(f"Best model training path: {best_model_path} (F1={best_f1:.4f})")

            final_weights_path = Path(output_dir) / "best_training_weights.pt"
    ...
    def _reset_loggers(self):
        """Reset metric loggers for new training step"""
        self.train_metric_logger = MetricLogger(delimiter="  ")
        self.train_metric_logger.add_meter('lr', SmoothedValue(window_size=1, fmt='{value:.6f}'))
        self.validation_metrics = {}
        self.end_time = time.time()
    ...
    def _set_trainable_layers(self, num_layers):
        """Set trainable layers for progressive training"""
        # First freeze all parameters
        for param in self.model.model.parameters():
            param.requires_grad = False
        
        if num_layers >= 3:  # train head, fpn, backbone
            for param in self.model.model.parameters():
                param.requires_grad = True
        
        elif num_layers >= 2:  # Train head, fpn 
        ...
        elif num_layers >= 1:  # Train only head
            for param in self.model.model.model[-1].parameters():
                param.requires_grad = True
    ...
    def _on_batch_end(self, trainer):
        ...
        if self.batch_idx == 1 or self.batch_idx % self.print_freq == 0 or self.batch_idx == self.num_batches:
    ...
    def _on_val_end(self, validator):
        try:
            ...
            map = float(val_metrics.box.map) if hasattr(val_metrics.box, 'map') else 0.0
            ...
            val_str = f"mAP50: {map50:.4f}  mAP: {map:.4f}  precision: {precision:.4f}  recall: {recall:.4f}  f1: {f1:.4f}  loss: {val_total_loss:.4f}"
            ...
            if self.ray_tune_callback:
                train_metrics = {
                    ...
                    'lr': self.train_metric_logger.meters['lr'].global_avg,
                }
